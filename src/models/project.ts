...
import { Project, CreateProjectRequest, UpdateProjectRequest } from "@parkour-ops/airetable-contract";
import { MarkOptional } from "ts-essentials";
import { v4 as uuidv4 } from "uuid";
import { undescribed, untitled } from "../generic";
import { Buildable } from "ts-essentials";

export async function readProject(userId: string, projectId: string) : Promise<Project | undefined> {
...
export async function createProject(userId: string, project: CreateProjectRequest) : Promise<Project> {
...
export async function updateProject(userId: string, project: UpdateProjectRequest) : Promise<Project> {
  return new Promise<Project>( async (res, rej)=>{
    // reject if project doesn't exist
    const projectToUpdate = await readProject(userId, project.id);
    if (!projectToUpdate)
      rej(new Error("Project to update does not exist!"));
    // update the project
    let err;
    await fb.db(`users/${userId}/projects/${project.id}`).update(project, (_err)=>{
      err = _err;
    });
    // reject if error
    if (err)
